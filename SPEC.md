...
- **`Cmd+J`** opens the journal home — every press, never a toggle. Pressed with the feed already showing it puts the caret in today's writing block instead of navigating anywhere, so holding it down doesn't cycle between two views. That request is explicit, so unlike the automatic "opens ready to write" it *may* take the caret from another day in the feed. Today's own page is reached by clicking its heading.
